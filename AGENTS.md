...
- `mise run install`: install this package and its dependencies.
...
- Python provisioning, dependency management, and command execution go through
  `uv`; `.python-version` is the interpreter source of truth.
- Hatchling builds the installed package from `src/gh_babysitter`.
