...
Один процесс FastAPI, четыре логических компонента. Общая схема — в [README](../../README.md#solution).
...
5. Dispatcher предлагает событие bounded-очереди каждого совпавшего `Subscriber`; успешное добавление считается доставкой, переполнение — потерей.
...
`Subscriber.offer(envelope)` не блокирует ingress: кладёт событие в очередь и возвращает `true`, либо при полной очереди увеличивает `dropped` и возвращает `false`. `take_dropped()` забирает и обнуляет счётчик; стрим превращает его в адресное событие `lag` для отставшего клиента ([delivery](02-delivery.md#переполнение-очереди)).
...
| Условие | HTTP | Тело |
|---|---:|---|
| HMAC отсутствует или не совпал | `401` | ошибка авторизации; тело запроса не парсится |
| После успешной HMAC-проверки тело не JSON | `400` | `{"detail":"Malformed JSON payload"}` |
| JSON-значение не объект | `400` | `{"detail":"Payload must be a JSON object"}` |
| Событие принято | `202` | `{"matched":3,"delivered":2,"dropped":1}` с фактическими счётчиками |
